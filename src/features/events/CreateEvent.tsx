  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dadosEvento = {
        ...
        ativo: true,
        config_pagamento: {
          pix:            { ativo: true, taxa: calcConfig?.pixFee ?? 1.99, tipo_taxa: 'fixo' },
          debito:         { ativo: true, taxa: calcConfig?.debitFee    ?? 1.99, tipo_taxa: 'porcentagem' },
          cartao_credito: { ativo: true, taxa: calcConfig?.creditFee   ?? 2.99, tipo_taxa: 'porcentagem', parcelas_max: 12 },
          corrente:       { ativo: true, taxa: calcConfig?.recurringFee ?? 4.99, tipo_taxa: 'porcentagem' },
          boleto:         { ativo: true, taxa: calcConfig?.boletoFee   ?? 1.99, tipo_taxa: 'fixo' },
